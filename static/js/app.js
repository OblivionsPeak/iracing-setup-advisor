...
function getExportToolbarHTML() {
  return '<div class="export-toolbar" id="export-toolbar">'
    + '<span class="export-toolbar-label">Export</span>'
    + '<button class="export-btn" onclick="exportAsImage()" title="Save current view as PNG image">'
    + '<span class="export-btn-icon">&#128247;</span> Export as Image'
    + '</button>'
    + '<button class="export-btn" onclick="printReport()" title="Open print dialog for PDF or paper">'
    + '<span class="export-btn-icon">&#128424;</span> Print Report'
    + '</button>'
    + '</div>';
}
...
function getExportHeader() {
  var carSel = document.getElementById('car-select');
  var trackSel = document.getElementById('track-select');
  var carName = carSel && carSel.selectedIndex > 0 ? carSel.options[carSel.selectedIndex].textContent : '';
  var trackName = trackSel && trackSel.selectedIndex > 0 ? trackSel.options[trackSel.selectedIndex].textContent : '';
  var date = new Date().toLocaleDateString('en-US', {year:'numeric', month:'short', day:'numeric'});
  var parts = ['<strong>iRacing Setup Advisor</strong>'];
  if (carName) parts.push(carName);
  if (trackName) parts.push(trackName);
  parts.push(date);
  return '<div class="export-header">' + parts.join(' &nbsp;|&nbsp; ') + '</div>';
}

function getExportWatermark() {
  return '<div class="export-watermark">Generated by iRacing Setup Advisor</div>';
}

function exportAsImage() {
  var results = document.getElementById('results');
  if (!results || results.style.display === 'none') {
    var stoOutput = document.getElementById('sto-analysis-output');
    if (stoOutput && stoOutput.style.display !== 'none') {
      results = stoOutput;
    } else {
      return;
    }
  ...
  if (compareResults && compareResults.innerHTML.trim()) {
    results = compareResults;
  }

  // Add export-mode class for light theme capture
  results.classList.add('export-mode');

  // Inject header and watermark
  var header = document.createElement('div');
  header.className = 'export-header';
  header.innerHTML = getExportHeader().replace(/<\/?div[^>]*>/g, '');
  results.insertBefore(header, results.firstChild);

  var watermark = document.createElement('div');
  watermark.className = 'export-watermark';
  watermark.textContent = 'Generated by iRacing Setup Advisor';
  results.appendChild(watermark);

  // Hide the export toolbar itself during capture
  ...
  html2canvas(results, {
    ...
    // Clean up
    results.classList.remove('export-mode');
    results.removeChild(header);
    results.removeChild(watermark);
    if (toolbar) toolbar.style.display = '';

    // Download
    var a = document.createElement('a');
    a.href = canvas.toDataURL('image/png');
    var fname = 'iracing-setup';
    if (window._analysisData && window._analysisData.meta && window._analysisData.meta.filename) {
      fname = window._analysisData.meta.filename.replace(/\.ibt$/i, '');
    }
    a.download = fname + '_export.png';
    a.click();
  }).catch(function(err) {
    // Clean up on error
    results.classList.remove('export-mode');
    if (header.parentNode) results.removeChild(header);
    if (watermark.parentNode) results.removeChild(watermark);
    if (toolbar) toolbar.style.display = '';
...
function render(data) {
  ...
  var html = getExportToolbarHTML() + autoDetectBanner + missingBanner + raceWarning + '<div class="meta-bar">'
